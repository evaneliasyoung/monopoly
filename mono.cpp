      ...
      { // Landed on a chance spot.
        ...
        { // -1 means a card does nothing.
          ...
          { // Move the player to the next railroad.
            pl = (((int)(std::round((float)(pl) / 10)) * 10) + 5) % SPACES;
          }
          else
...
int main(int argc, char const *argv[])
{
  ...
  std::cout << "MONOPOLY-C++" << '\n'
            << "RUNNING " << std::to_string(GAMES) << " GAMES" << '\n'
            << "PLAYING " << std::to_string(MOVES) << " MOVES" << '\n';
  ...
  {
    std::cout << std::to_string(turn_lands[i]) << '\n';
  }
  std::cout << "ENDS" << '\n';
  for (size_t i = 0; i < SPACES; ++i)
  {
    std::cout << std::to_string(turn_ends[i]) << '\n';
  }
